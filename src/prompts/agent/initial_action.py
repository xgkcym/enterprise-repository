...
3. 如果问题是通用解释、润色、翻译、总结、概念问答，且不依赖企业内部知识或实时信息，优先 direct_answer。
4. 如果问题是企业内部知识问答且表达清晰，优先 rag。
5. 如果问题表达模糊，优先 rewrite_query。
6. 如果问题复杂且包含多个目标，优先 decompose_query。
7. 如果关键上下文缺失，优先 clarify_question。
